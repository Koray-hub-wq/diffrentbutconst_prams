...
paths explicitly on the server.

Example:

python constant_parameter_stochastic_median_slides_server.py \
  --repo-root /path/to/DynaMix-python-b-tipping \
  --data-dir /path/to/generate_trainingdata/diffrentbutconst_prams/14to209_noisy/windows \
  --with-phi-run /path/to/DynaMix-python-b-tipping/results/single_halvorsen_constparams/14to209_noisy-with-phi \
  --no-phi-run /path/to/DynaMix-python-b-tipping/results/single_halvorsen_constparams/14to209_noisy-no-phi \
  --output-dir /path/to/output/median_stochastic_slides \
  --device cuda
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "CUDA device index to use. If set with --device cuda, the effective "
            "device becomes cuda:<gpu-id>."
        ),
    )
    parser.add_argument(
        "--torch-threads",
        type=int,
        default=4,
        help="Set torch CPU intra-op threads and common BLAS thread env vars.",
    )
    parser.add_argument(
        "--torch-interop-threads",
        type=int,
        default=1,
        help="Set torch CPU inter-op threads.",
    )
...
def configure_runtime(args: argparse.Namespace) -> str:
    ...
    if args.torch_interop_threads is not None:
        if args.torch_interop_threads <= 0:
            raise ValueError("torch_interop_threads must be positive")
    ...
    if args.gpu_id is not None:
        ...
        if device == "cuda":
            ...
            raise ValueError(
                f"--gpu-id is only supported with cuda devices, got {device}"
            )
...
def load_test_data(data_dir: Path, context_steps_override: int | None):
    ...
    context_steps = (
        int(context.shape[0])
        if context_steps_override is None
        else context_steps_override
    )
    if test.ndim != 3:
...
def select_group_median(
    ...
    selected["prediction"] = preds[selected["rollout_idx"]][
        :, selected["series_idx"], :
    ]
...
def full_prediction(
    test: np.ndarray, pred_future: np.ndarray, series_idx: int, context_steps: int
):
...
def main() -> None:
    ...
    for rollout_idx in range(args.rollouts):
        ...
        no_preds.append(
            forecast_once(
                no_model, no_forecaster, test, test_phi, context_steps, device
            )
        )
        phi_preds.append(
            forecast_once(
                phi_model, phi_forecaster, test, test_phi, context_steps, device
            )
        )
